...
export const Sidebar: React.FC<SidebarProps> = () => {
  ...
  return <div style={sidebarStyle}>
      <button style={toggleButtonStyle} onClick={() => setIsCollapsed(!isCollapsed)} onMouseEnter={e => {
      ...
      <div style={headerStyle} className="px-[20px] py-[10px] my-0 mx-0">
        <div style={logoStyle}>
          {isCollapsed ? '💎' : '💎 Crystal'}
        </div>
      </div>
      ...
      <nav style={menuStyle}>
        ...
        return <div key={item.id} style={menuItemStyle(isActive)} onClick={() => navigate(item.path)} onMouseEnter={e => {
          if (!isActive) {
            e.currentTarget.style.backgroundColor = 'hsl(var(--accent))';
          }
        }} onMouseLeave={e => {
          if (!isActive) {
            e.currentTarget.style.backgroundColor = 'transparent';
              ...
              {!isCollapsed && <div style={labelContainerStyle}>
